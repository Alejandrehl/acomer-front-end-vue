<template>
  <Modal
    ...
    :show="showModal"
    @close-modal="toggle_modal_state({name:'order_detail', value: false})"
  >
    <div class="field" v-if="order.id">
      <h1>
        Orden en
        <nuxt-link
          :to="{name: 'sucursales-id', params: { id: order.branch_id } }"
        >{{order.branch.name}}</nuxt-link>
      </h1>
...
export default {
  ...
  watch: {
    order_id: function() {
      self = this;
      api.get(`api/orders/${this.order_id}`).then(function(response) {
